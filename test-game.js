...
// 模拟游戏逻辑
const gameState = {
    age: 0, health: 100, money: 0, intelligence: 50, charm: 50, luck: 50, fame: 0,
    skills: [], events: [], job: "婴儿", relationship: 0, careerLevel: 0, history: []
};
...
function applyEffect(effect) {
    ...
    if (effect.skill && !gameState.skills.includes(effect.skill)) gameState.skills.push(effect.skill);
...
function testGame() {
    ...
    while (gameState.age < 65) {
        ...
        // 获取当前事件
        const evt = events[currentAge] || generateDefaultEvent(currentAge);
        
        if (!evt.choices || evt.choices.length === 0) {
            console.log(`❌ 年龄 ${currentAge}: 没有选项！`);
            return false;
        }
        ...
        if (validChoices.length === 0) {
            console.log(`❌ 年龄 ${currentAge}: 没有有效的选项！`);
            return false;
        }
        ...
        // 记录事件
        gameState.events.push({ age: currentAge, event: choice.text });
        
        // 检查游戏结束条件
        if (gameState.health <= 0) {
            console.log(`💀 年龄 ${currentAge}: 健康耗尽，游戏结束！`);
            return false;
        }
        if (gameState.money < -50000) {
            console.log(`💸 年龄 ${currentAge}: 负债过多，游戏结束！`);
            return false;
        }
        ...
        // 打印进度（每5年或关键年龄打印）
        if (currentAge % 5 === 0 || currentAge === 0 || currentAge === 3 || currentAge === 6 || 
            currentAge === 9 || currentAge === 12 || currentAge === 15 || currentAge === 18 ||
            currentAge === 21 || currentAge === 60) {
            console.log(`✅ 年龄 ${currentAge} → ${gameState.age}岁: ${choice.text}`);
            console.log(`   健康:${gameState.health} 金钱:¥${gameState.money} 智力:${gameState.intelligence} 魅力:${gameState.charm}`);
        }
    }
